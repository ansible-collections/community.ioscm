...
class TerminalModule(TerminalBase):
    ...
    terminal_stderr_re = [
        ...
        re.compile(rb"(?:incomplete|ambiguous) command", re.IGNORECASE),
        re.compile(rb"connection timed out", re.IGNORECASE),
        re.compile(rb"[^\r\n]+ not found"),
        re.compile(rb"'[^']' +returned error code: ?\d+"),
        re.compile(rb"Bad mask", re.IGNORECASE),
        re.compile(rb"% ?(\S+) ?overlaps with ?(\S+)", re.IGNORECASE),
        re.compile(rb"[%\S] ?Error: ?[\s]+", re.IGNORECASE),
        re.compile(rb"[%\S] ?Informational: ?[\s]+", re.IGNORECASE),
        re.compile(rb"Command authorization failed"),
        re.compile(rb"Command Rejected: ?[\s]+", re.IGNORECASE),
        re.compile(rb"% General session commands not allowed under the address family", re.IGNORECASE),
        re.compile(rb"% BGP: Error initializing topology", re.IGNORECASE),
    ]
